(() => {
  ...
  const CNN_FEAR_GREED_PROXY_PATH = "/api/cnn-fear-greed";
  ...
  function toValidClosePoints(result) {
  ...
  function resolveDailySnapshot(result) {
    const points = toValidClosePoints(result);
    const latestPoint = points.at(-1) || null;
    const previousPoint = points.at(-2) || null;
    const regularMarketPrice = result?.meta?.regularMarketPrice;
    const latestPrice =
      typeof regularMarketPrice === "number" && !Number.isNaN(regularMarketPrice)
        ? regularMarketPrice
        : latestPoint?.close ?? null;
    const metaPreviousClose = result?.meta?.previousClose;
    const previousClose =
      typeof metaPreviousClose === "number" && !Number.isNaN(metaPreviousClose)
        ? metaPreviousClose
    ...
    return {
      latestPrice,
      previousClose,
      lastUpdated: latestPoint ? new Date(latestPoint.timestamp * 1000).toISOString() : null,
    };
  }
  ...
  function buildYahooChartPayload(basePayload, url) {
    ...
    const dailySnapshot = resolveDailySnapshot(result);

    let indices = timestamps.map((_, index) => index);
    if (rangeDays != null) {
      const cutoff = latestTimestamp - rangeDays * 86400;
    ...
    const nextResult = {
      ...
      meta: {
        ...
        regularMarketPrice: dailySnapshot.latestPrice,
        previousClose: dailySnapshot.previousClose,
        chartPreviousClose: dailySnapshot.previousClose,
      },
      timestamp: indices.map((index) => timestamps[index]),
      indicators: {
        ...result.indicators,
  ...
  function applyMarketData(items) {
    ...
    cards.forEach((card) => {
      ...
      badgeElement.textContent = `${isUp ? "UP" : "DOWN"} ${formatChangePercent(item)}`;
    });
  }
  ...
  function filterVkospiOutput(payload, range) {
    ...
    const filteredRows = allRows.filter((row) => {
      ...
      const dayDiff = (latestDate.getTime() - rowDate.getTime()) / 86400000;
